    ...
    def load_groups_from_zip_archive(self, zip_file):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # If only one candidate dir and it is the extraction root, treat entire archive as a single group
                if len(candidate_dirs) == 1 and candidate_dirs[0][0] == temp_dir:
                    inferred_name = getattr(zip_file, 'name', 'archive').rsplit('.', 1)[0]
                    candidate_dirs = [(candidate_dirs[0][0], candidate_dirs[0][1], inferred_name)]
                else:
