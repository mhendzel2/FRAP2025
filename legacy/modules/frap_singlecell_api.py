...
try:
    ...
    BAYES_BACKEND_AVAILABLE = True
except Exception:
    BAYES_BACKEND_AVAILABLE = False
...
def fit_hierarchical_bayes_from_tables(
    roi_traces: pd.DataFrame,
    cell_features: pd.DataFrame,
    ...
    use_jax: bool = False,
    interpolate_to_common_grid: bool = False,
    common_grid_size: Optional[int] = None,
) -> dict:
    """
    Fit a hierarchical Bayesian FRAP model from existing analysis tables.

    Returns a dictionary with the prepared dataset and fit result.
    """
    if not BAYES_BACKEND_AVAILABLE:
        raise ImportError(
            "Bayesian backend is unavailable. Ensure frap_bayes_* modules and dependencies are installed."
        )
    ...
    recipe = BayesianPreprocessingRecipe(
        ...
        interpolation="common_grid" if interpolate_to_common_grid else "none",
        common_grid_size=common_grid_size,
        qc_only=True,
        notes={"source": "frap_singlecell_api"},
    )
    ...
    dataset = build_bayesian_dataset(
        ...
        recipe=recipe,
        interpolate_to_common_grid=interpolate_to_common_grid,
        common_grid_size=common_grid_size,
    )
    ...
    fit_result = fit_hierarchical_bayes(
        ...
        use_jax=use_jax,
        init_from_cell_features=cell_features,
    )
