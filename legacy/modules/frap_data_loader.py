...
def validate_and_load(roi_traces: pd.DataFrame, cell_features: pd.DataFrame):
    ...
    def _coerce_validation(result) -> tuple[bool, str]:
        if isinstance(result, tuple) and len(result) == 2:
            return bool(result[0]), str(result[1])
        valid = bool(result)
        return valid, "ok" if valid else "Schema validation failed"

    # Validate schema
    try:
        traces_valid, traces_msg = _coerce_validation(
            validate_roi_traces(roi_traces, return_message=True)
        )
    except TypeError:
        traces_valid, traces_msg = _coerce_validation(validate_roi_traces(roi_traces))
    if not traces_valid:
        st.error(f"Invalid ROI traces: {traces_msg}")
        raise ValueError(traces_msg)
    
    try:
        features_valid, features_msg = _coerce_validation(
            validate_cell_features(cell_features, return_message=True)
        )
    except TypeError:
        features_valid, features_msg = _coerce_validation(validate_cell_features(cell_features))
