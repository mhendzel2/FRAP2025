"""
Advanced group statistics for FRAP analysis
Includes permutation tests, effect sizes, FDR correction, TOST, and mixed-effects models
"""
import pandas as pd
import numpy as np
...
def _calculate_effect_sizes(data, metric, group1, group2):
    ...
    try:
        ...
        logging.warning(f"Effect size calculation failed: {e}")
        cohen_d, cliffs_delta = np.nan, np.nan
...
def _tost_equivalence(data, metric, group1, group2, low_eq_bound, high_eq_bound):
    ...
    try:
        ...
        logging.warning(f"TOST calculation failed: {e}")
        p_val, result = np.nan, "Error"
...
def calculate_group_stats(
    ...
    -----------
    data : pd.DataFrame
        Data with a 'group' column and metric columns
    metrics : list
        List of metric column names to analyze
    group_order : list
        Ordered list of group names
    tost_thresholds : dict, optional
        Dictionary mapping metrics to (low, high) equivalence bounds
    use_mixed_effects : bool
        Whether to fit mixed-effects models
    ...
    --------
    pd.DataFrame
        Results dataframe with statistical tests and effect sizes
    """
    results = []
    ...
    if not results_df.empty and 'p_value' in results_df.columns:
        ...
        if len(p_values) > 0:
            ...
            results_df.loc[results_df['p_value'].notna(), 'q_value'] = pvals_corrected
