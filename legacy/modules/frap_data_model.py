...
class DataIO:
    ...
    def save_bayesian_outputs(
        ...
        inference_data : Any, optional
            ArviZ InferenceData-compatible object with `to_netcdf`.
        summary_tables : dict[str, pd.DataFrame], optional
            Named summary tables written to CSV.
        metadata : dict, optional
            Additional metadata written to JSON.
        prefix : str
            File prefix.
        ...
        if inference_data is not None:
            ...
            if hasattr(inference_data, "to_netcdf"):
                inference_data.to_netcdf(idata_path)
                saved["inference_data"] = idata_path
            else:
                raise TypeError("inference_data must implement to_netcdf(path)")

        if summary_tables:
            for table_name, table_df in summary_tables.items():
                if table_df is None:
                    continue
                safe_name = str(table_name).replace(" ", "_")
                table_path = output_dir / f"{prefix}_{safe_name}.csv"
                table_df.to_csv(table_path, index=False)
                saved[f"table_{safe_name}"] = table_path
        ...
        logger.info("Saved Bayesian artifacts to %s", output_dir)
        return saved

    @staticmethod
    def load_bayesian_outputs(
        input_dir: str | Path,
        prefix: str = "bayes",
    ) -> dict[str, Any]:
        """
        Load Bayesian outputs previously saved with `save_bayesian_outputs`.
        """
        input_dir = Path(input_dir)
        ...
        idata_path = input_dir / f"{prefix}_idata.nc"
        if idata_path.exists():
            try:
                import arviz as az

                outputs["inference_data"] = az.from_netcdf(idata_path)
            except Exception:
                outputs["inference_data"] = idata_path

        for csv_path in input_dir.glob(f"{prefix}_*.csv"):
            stem = csv_path.stem
            table_name = stem[len(prefix) + 1 :]
            outputs["summary_tables"][table_name] = pd.read_csv(csv_path)

        meta_path = input_dir / f"{prefix}_metadata.json"
        if meta_path.exists():
            with open(meta_path, "r", encoding="utf-8") as fh:
                outputs["metadata"] = json.load(fh)
...
def validate_roi_traces(
    ...
    if missing:
        message = f"Missing columns in roi_traces: {sorted(missing)}"
...
def validate_cell_features(
    ...
    missing = set(required_cols) - set(df.columns)
    if missing:
        message = f"Missing columns in cell_features: {sorted(missing)}"
        logger.error(message)
        if return_message:
            return False, message
        return False
