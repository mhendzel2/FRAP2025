...
# Optional hierarchical Bayesian global fitting backend
try:
    ...
    from frap_bayes_reports import build_bayesian_report_tables

    HB_GLOBAL_FITTING_AVAILABLE = True
    HB_GLOBAL_FITTING_IMPORT_ERROR = ""
except Exception as hb_global_import_error:
    HB_GLOBAL_FITTING_AVAILABLE = False
    HB_GLOBAL_FITTING_IMPORT_ERROR = str(hb_global_import_error)
    logger.warning("Hierarchical Bayesian global fitting not available: %s", HB_GLOBAL_FITTING_IMPORT_ERROR)
...
def apply_advanced_fitting(t: np.ndarray, intensity: np.ndarray, 
    ...
    try:
        ...
        # k is the fitted exponential rate (k = 1/tau), not the reciprocal half-time.
                ...
                with st.expander("🌐 Global Simultaneous Fit", expanded=False):
                    ...
                    **Global fitting** constrains kinetic rate constants to be identical across all traces
                    ...
                    with col_global1:
                        ...
                        enable_bayesian_models = st.checkbox(
                            "Enable hierarchical Bayesian models",
                            value=False,
                            disabled=not HB_GLOBAL_FITTING_AVAILABLE,
                            help=(
                                "Adds HB model options powered by PyMC. "
                                "Use this when you want condition-level shrinkage and posterior uncertainty."
                            )
                        )
                        if not HB_GLOBAL_FITTING_AVAILABLE:
                            st.caption(
                                "Hierarchical Bayesian backend unavailable in this environment: "
                                f"{HB_GLOBAL_FITTING_IMPORT_ERROR}"
                            )
                        ...
                        if enable_bayesian_models and HB_GLOBAL_FITTING_AVAILABLE:
                            global_model_options.extend(
                                ["hb_1exp", "hb_2exp_ordered", "hb_stretched", "hb_reaction_dominant"]
                            )
                        ...
                        model_labels = {
                            ...
                            "hb_1exp": "HB 1-exp (PyMC NUTS)",
                            "hb_2exp_ordered": "HB 2-exp ordered (PyMC NUTS)",
                            "hb_stretched": "HB stretched exponential (PyMC NUTS)",
                            "hb_reaction_dominant": "HB reaction-dominant proxy (PyMC NUTS)",
                        }
                        ...
                        is_bayesian_model = str(global_model).startswith("hb_")
                        bayes_mode = "deterministic"
                        bayes_draws = 600
                        bayes_tune = 600
                        bayes_chains = 2
                        bayes_target_accept = 0.9
                        bayes_use_jax = False
                        bayes_interpolate_common_grid = False
                        bayes_common_grid_size = 60
                        ...
                        if is_bayesian_model:
                            ...
                            bayes_mode = st.radio(
                                ...
                                horizontal=True,
                                help="Deterministic fits corrected traces; semi-generative adds nuisance correction parameters."
                            )
                            bayes_draws = int(st.slider("Draws", 200, 2000, 600, 100))
                            bayes_tune = int(st.slider("Tune", 200, 2000, 600, 100))
                            bayes_chains = int(st.slider("Chains", 2, 4, 2, 1))
                            bayes_target_accept = float(st.slider("Target accept", 0.80, 0.99, 0.90, 0.01))
                            bayes_use_jax = st.checkbox(
                                "Use JAX/Numpyro sampler",
                                value=False,
                                help="Can improve speed on larger datasets when JAX backend is available."
                            )
                            bayes_interpolate_common_grid = st.checkbox(
                                "Interpolate traces to a common time grid",
                                value=False,
                                help="Useful when traces are strongly ragged."
                            )
                            if bayes_interpolate_common_grid:
                                bayes_common_grid_size = int(
                                    st.number_input(
                                        "Common grid points",
                                        min_value=8,
                                        max_value=300,
                        ...
                        if st.button("🚀 Run Global Fit (Configured)", type="primary", disabled=(len(included_groups) == 0)):
                            try:
                                ...
                                with st.spinner(f"Performing global fitting ({model_label})..."):
                                    ...
                                    failed = []
                                    ok_groups = []
                                    global_fit_summary_df = pd.DataFrame()
                                    ...
                                    if is_bayesian_model:
                                        if not HB_GLOBAL_FITTING_AVAILABLE:
                                            raise RuntimeError(
                                                "Hierarchical Bayesian backend is unavailable. "
                                                f"Import error: {HB_GLOBAL_FITTING_IMPORT_ERROR}"
                                            )
                                        ...
                                        for gname in included_groups:
                                            excluded_group = set(_excluded_for_group(gname))
                                            group_files = (dm.groups.get(gname, {}) or {}).get('files', []) or []
                                            for file_path in group_files:
                                                if file_path in excluded_group or file_path not in dm.files:
                                                    continue
                                                ...
                                                finite_mask = np.isfinite(t_post) & np.isfinite(i_post)
                                                t_post = np.asarray(t_post, dtype=float)[finite_mask]
                                                i_post = np.asarray(i_post, dtype=float)[finite_mask]
                                                if len(t_post) < 4:
                                                    continue

                                                cell_counter += 1
                                                movie_id = str(file_data.get('name', f"trace_{cell_counter}"))
                                                for frame_idx, (tt, yy) in enumerate(zip(t_post, i_post)):
                                                    hb_trace_rows.append({
                                                        'exp_id': str(gname),
                                                        'movie_id': movie_id,
                                                        'cell_id': int(cell_counter),
                                                        'frame': int(frame_idx),
                                                        't': float(tt),
                                                        'signal_norm': float(yy),
                                                        'signal_corr': float(yy),
                                                        'radius': float(st.session_state.settings.get('default_bleach_radius', 1.0)),
                                                    })

                                                feat = (file_data.get('features') or {})
                                                hb_feature_rows.append({
                                                    'exp_id': str(gname),
                                                    'movie_id': movie_id,
                                                    'cell_id': int(cell_counter),
                                                    'condition': str(gname),
                                                    'bleach_qc': True,
                                                    'k': float(feat.get('koff', feat.get('k', np.nan))),
                                                    'I0': float(feat.get('I0', 0.0)),
                                                    'I_inf': float(feat.get('I_inf', 1.0)),
                                                })

                                        if len(hb_feature_rows) < 2:
                                            raise ValueError("Need at least 2 traces to run hierarchical Bayesian fitting.")

                                        hb_traces_df = pd.DataFrame(hb_trace_rows)
                                        hb_features_df = pd.DataFrame(hb_feature_rows)
                                        hb_recipe = BayesianPreprocessingRecipe(
                                            mode=bayes_mode,
                                            intensity_source='signal_norm',
                                            fallback_intensity_source='signal_corr',
                                            interpolation='common_grid' if bayes_interpolate_common_grid else 'none',
                                            common_grid_size=bayes_common_grid_size if bayes_interpolate_common_grid else None,
                                            qc_only=True,
                                            notes={'source': 'streamlit_frap_global_menu'},
                                        )

                                        hb_dataset = build_bayesian_dataset(
                                            roi_traces=hb_traces_df,
                                            cell_features=hb_features_df,
                                            recipe=hb_recipe,
                                            interpolate_to_common_grid=bayes_interpolate_common_grid,
                                            common_grid_size=bayes_common_grid_size if bayes_interpolate_common_grid else None,
                                        )
                                        ...
                                        hb_fit = fit_hierarchical_bayes(
                                            ...
                                            backend='pymc',
                                            model_config={'mode': bayes_mode},
                                            draws=bayes_draws,
                                            tune=bayes_tune,
                                            chains=bayes_chains,
                                            target_accept=bayes_target_accept,
                                            use_jax=bayes_use_jax,
                                            init_from_cell_features=hb_features_df,
                                            sample_prior_predictive_draws=200,
                                            sample_posterior_predictive=True,
                                        )

                                        hb_tables = build_bayesian_report_tables(
                                            hb_fit.idata,
                                            parameters=['k', 'k_slow', 'k_fast', 'tau', 'k_off', 'I0', 'I_inf', 'sigma'],
                                        )
                                        global_fit_summary_df = hb_tables.get('parameter_summary', pd.DataFrame())
                                        if not global_fit_summary_df.empty and 'parameter' in global_fit_summary_df.columns:
                                            global_fit_summary_df.insert(0, 'Model', global_model)

                                        ok_groups = list(hb_dataset.condition_levels)
                                        failed = [g for g in included_groups if g not in set(ok_groups)]
                                        if failed:
                                            st.warning(
                                                "Bayesian fit skipped groups with no valid traces after filtering: "
                                                f"{', '.join(failed)}"
                                            )
                                        st.success(
                                            "✅ Hierarchical Bayesian fitting completed "
                                            f"({hb_dataset.n_cells} traces, {hb_dataset.n_obs} observations)."
                                        )

                                        bayes_payload = {
                                            'idata': hb_fit.idata,
                                            'summary_tables': hb_tables,
                                            'recipe': hb_dataset.recipe,
                                            'n_obs': hb_dataset.n_obs,
                                            'n_cells': hb_dataset.n_cells,
                                            'n_conditions': hb_dataset.n_conditions,
                                            'diagnostics': hb_fit.diagnostics,
                                            'information_criteria': hb_fit.information_criteria,
                                            'ppc_metrics': hb_fit.ppc_metrics,
                                            'condition_effects_df': hb_tables.get('condition_effects', pd.DataFrame()),
                                            'settings': {
                                                'mode': bayes_mode,
                                                'draws': bayes_draws,
                                                'tune': bayes_tune,
                                                ...
                                                'target_accept': bayes_target_accept,
                                                'use_jax': bayes_use_jax,
                                                'interpolate_common_grid': bayes_interpolate_common_grid,
                                                'common_grid_size': bayes_common_grid_size if bayes_interpolate_common_grid else None,
                                            },
                                        }
                                    else:
                                    ...
                                    if ok_groups:
                                        st.session_state.global_fit_last_run = {
                                            ...
                                            'is_bayesian': bool(is_bayesian_model),
                                            ...
                                            'stats_df': stats_df,
                                            'bayes_payload': bayes_payload if is_bayesian_model else {},
                                        }

                                    # Optional report generation using same configuration
                                    if auto_make_reports and is_bayesian_model:
                                        st.info(
                                            "Auto report generation is currently available for deterministic global fits. "
                                            "Bayesian summary outputs are preserved in the Last Run panel."
                                        )
                                    elif auto_make_reports:
                    ...
                    if 'global_fit_last_run' in st.session_state:
                        ...
                        with st.expander("📌 Last Global Fit Run Results", expanded=True):
                            ...
                            if bool(last.get('is_bayesian', False)):
                                bayes_payload = last.get('bayes_payload') or {}
                                st.markdown("#### Bayesian Diagnostics")
                                diag = bayes_payload.get('diagnostics') or {}
                                ic = bayes_payload.get('information_criteria') or {}
                                ppc = bayes_payload.get('ppc_metrics') or {}

                                dcol1, dcol2, dcol3, dcol4 = st.columns(4)
                                with dcol1:
                                    st.metric("Cells", bayes_payload.get('n_cells', np.nan))
                                with dcol2:
                                    st.metric("Observations", bayes_payload.get('n_obs', np.nan))
                                with dcol3:
                                    max_rhat = diag.get('max_rhat', np.nan)
                                    st.metric("Max R-hat", f"{max_rhat:.3f}" if np.isfinite(max_rhat) else "n/a")
                                with dcol4:
                                    st.metric("Divergences", diag.get('divergences', 0))

                                if 'loo' in ic and isinstance(ic.get('loo'), dict):
                                    loo = ic.get('loo', {})
                                    st.caption(
                                ...
                                if ppc:
                                    st.caption(
                                        f"PPC RMSE={ppc.get('rmse', np.nan):.4f}, "
                                        f"MAE={ppc.get('mae', np.nan):.4f}, "
                                        f"95% coverage={ppc.get('coverage_95', np.nan):.3f}"
                                    )

                                cond_df = bayes_payload.get('condition_effects_df')
                                if isinstance(cond_df, pd.DataFrame) and not cond_df.empty:
                                    st.markdown("#### Condition Effects (Posterior)")
                                    st.dataframe(cond_df, width="stretch")

                                st.markdown("#### Export Bayesian Artifacts")
                                run_stamp = str(last.get('timestamp', pd.Timestamp.now().isoformat()))
                                safe_stamp = run_stamp.replace(":", "").replace("T", "_").replace("-", "").split(".")[0]
                                default_export_dir = os.path.join("output", f"bayes_global_{safe_stamp}")
                                export_dir_key = f"global_bayes_export_dir_{safe_stamp}"
                                save_btn_key = f"global_bayes_save_btn_{safe_stamp}"
                                ...
                                if st.button("Save Bayesian Artifacts", key=save_btn_key):
                                    try:
                                        saved_paths = DataIO.save_bayesian_outputs(
                                            output_dir=export_dir,
                                            inference_data=bayes_payload.get('idata'),
                                            summary_tables=bayes_payload.get('summary_tables'),
                                            metadata={
                                                'timestamp': last.get('timestamp'),
                                                'model': last.get('global_model'),
                                                'included_groups': last.get('included_groups', []),
                                                'ok_groups': last.get('ok_groups', []),
                                                'failed_groups': last.get('failed_groups', []),
                                                'bayesian_settings': bayes_payload.get('settings', {}),
                                                'diagnostics': diag,
                                        ...
                                        for artifact_name, artifact_path in saved_paths.items():
                                            st.write(f"- {artifact_name}: {artifact_path}")
                                    except Exception as save_exc:
                                        st.error(f"Failed to save Bayesian artifacts: {save_exc}")
                                st.caption(
                                    "Trace-level overlay plots in this panel are currently for deterministic global fits. "
                                    "Use the summary tables above for Bayesian runs."
                                )

                            show_plots = False
                            if not bool(last.get('is_bayesian', False)):
